...
| Scope | `.github/workflows/ci.yml` (future execution), `openspec/changes/ci-pr-verification-gate/` (planning), documentation |
...
- **Generalized Archive Guard:** Uses Git diff filters (`git diff --no-renames --diff-filter=MDT`) to block any modification or deletion of existing archive files while permitting legitimate new archive additions.
...
|------|--------|-------------|
| `.github/workflows/ci.yml` | NEW (in implementation phase) | Minimal 4-job single-file workflow |
| `src/clinical_compiler/` | NO CHANGE | Zero code modifications permitted |
| `tests/` | NO CHANGE | Existing test suite is canonical and unchanged |
...
- **AC6 (Archive Immutability):** Modifications or deletions of existing `openspec/changes/archive/` files fail the `governance` job.
- **AC7 (Archive Extensibility):** Addition of new archive directories in `openspec/changes/archive/` is permitted and passes.
- **AC8 (Least Privilege):** Workflow uses strict `permissions: contents: read`.
...
- **AC15 (PR Diff Hygiene):** Whitespace corruption or merge-conflict markers introduced by the PR fail the `governance` job via `git diff --check "${BASE_SHA}"...HEAD`.
